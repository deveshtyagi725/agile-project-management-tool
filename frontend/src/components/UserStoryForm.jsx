...
export const UserStoryForm = ({ initialData, onSubmit, onCancel, isLoading = false }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <FormGroup label="Story Title" error={errors.title} required>
        <Input
          type="text"
          name="title"
      ...
      <div className="flex gap-3 justify-end pt-4">
        ...
        <Button type="submit" disabled={isLoading}>
          {isLoading ? 'Saving...' : initialData?.id ? 'Update Story' : 'Create Story'}
        </Button>
      </div>
    </form>
  )
